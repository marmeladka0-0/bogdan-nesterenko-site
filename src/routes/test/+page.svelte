...
<main class="mx-auto w-full bg-black px-4 md:px-8  md:py-24">
	...
	<section id="videos" class="relative py-24 bg-black overflow-hidden group/section mx-auto w-full max-w-4xl">
					...
					<div class="video-wrapper relative aspect-video w-full  transition-all duration-700">
						...
						{:else}
							<img
								...
								alt={video.title}
								class="w-full h-full object-cover transition-all duration-300 rounded-[0.5rem]"
							/>
		...
		<button 
			class="nav-prev absolute left-4 md:left-10 top-[40%] z-30 opacity-0 group-hover/section:opacity-100 transition-all duration-300 hidden md:flex text-white/50 hover:text-white" 
		...
		<button 
			class="nav-next absolute right-4 md:right-10 top-[40%] z-30 opacity-0 group-hover/section:opacity-100 transition-all duration-300 hidden md:flex text-white/50 hover:text-white" 
			...
			<div class="w-full flex-1 flex flex-col justify-center py-4">
				<div class="relative space-y-6 font-sans text-[13px] leading-relaxed text-white/60 font-light tracking-wide text-justify border-r border-white/10 pr-8 md:text-right">
					...
					<p>
						...
						enregistrement des airs de Bach « Comme un air de passions...» (ed. AR-RE-
						SE) a reçu des critiques élogieuses. En duo avec Marc Hervieux (flûtes à bec) il
